...
def Song_str(S:Song)->str:
    '''takes the information from a song namedtuple and turns it into a readable string'''
    if(S.length%60 >= 10):
        return "Track Number:{:2}  Title:{:35}  length:{:5}  Play Count:{} \n".format(str(S.track),S.title,str(S.length//60) + ":" + str(S.length%60),S.play_count)
    return "Track Number:{:2}  Title:{:35}  length:{:5}  Play Count:{} \n".format(str(S.track),S.title,str(S.length//60) + ":0" + str(S.length%60),S.play_count)
def Album_str(A:Album)->str:
    '''takes the information from an album namedtuple and turns it into a readable string'''
    ret_str = ""
    ret_str += "Album ID:{:2}  Artist:{:20}  Title:{:20}  Year:{:5}\nThe list of songs in the Album:\n".format(A.id, A.artist,A.title,A.year)
    for song in A.songs:
        ret_str += Song_str(song)
    return ret_str
def Songdisplay_str(S:Songdisplay) ->str:
    '''takes the information from a song namedtuple and turns it into a readable string'''
    if(S.length%60 >= 10):
        return "Artist:{:20} Album:{:20}  Year:{:5} Track number:{:2} Title:{:35} length:{:5}  Play Count:{} \n".format(S.artist,S.a_title,S.year,str(S.track),S.s_title,str(S.length//60) + ":" + str(S.length%60),S.play_count)
    return "Artist:{:20} Album:{:20}  Year:{:5} Track number:{:2} Title:{:35} length:{:5}  Play Count:{} \n".format(S.artist,S.a_title,S.year,str(S.track),S.s_title,str(S.length//60) + ":0" + str(S.length%60),S.play_count)
print(Album_str(MUSIC[0]))
print()
print()
print('----part e.2----')
def Album_track_num(A:Album)->int:
    '''returns the number of songs in an album'''
    return len(A.songs)
MUSIC.sort(key = Album_track_num)
for A in MUSIC:
    print( Album_str(A))
print()
print()
...
def unplayed_songs(mc:'list of Album') ->'list of Songdisplay':
    '''makes a list of songs that have 0 plays'''
    song_list = []
    for A in mc:
        for songdisplay in Album_to_Songdisplays(A):
            if songdisplay.play_count == 0:
                song_list.append(songdisplay)
    return song_list
print('The list of unplayed songs:')
...
def length_from_songdisplay(S:Songdisplay)->int:
    '''returns the length of a song display'''
    return S.length
assert length_from_songdisplay(Songdisplay('hehe', "Once Upon a Time",2005,2,'burn', 345, 9))== 345

print('the length of the first song in album number 2 is',length_from_songdisplay(Album_to_Songdisplays(MUSIC[0])[0]))
print()
print()
print('----part e.5----')
def Song_listening_time(S:Song)->int:
    ''' takes a single song and returns the total number
    of seconds the user has spent listening to it '''
    return S.length * S.play_count
def Album_listening_time(A:Album)->int:
    ''' takes a single Album and returns the total number
    of seconds the user has spent listening to it '''
    alb_sum = 0
    for song in A.songs:
        alb_sum += Song_listening_time(song)
    return alb_sum
def favorite_album(AL:'list of Album')->Album:
    ''' takes a list of albums and returns the album that has the most total time spent listening to an album'''
    fav_alb = AL[0]
    for alb in AL:
        if Album_listening_time(alb) >  Album_listening_time(fav_alb):
            fav_alb = alb
    return fav_alb
print("The most popular Album is:\n\n",Album_str(favorite_album(MUSIC)))
print()
print()
print('----part e.6----')
def top_n(AL:'list of Album',num:int,f:str,B:bool)->'list of Songdisplay':
    '''takes a list of albums a number,a function, and a sortkey'''
    list_of_Songdisplays = all_Songdisplays(AL)
    list_of_Songdisplays.sort(key=f, reverse=B)
    return list_of_Songdisplays[:num]
print("The 3 most played songs are:")
for song in top_n(MUSIC, 3, play_count_from_songdisplay, True):
    print(Songdisplay_str(song))
print("\n\n")
print("The ten shortest songs are:")
for song in top_n(MUSIC, 10, length_from_songdisplay, False):
    print(Songdisplay_str(song))
print()
print()
print('----part e.7----')
def Album_PT(A:Album)->int:
    alb_sum = 0
    for song in A.songs:
        alb_sum += song.play_count
    return alb_sum
def favorite_album2(AL:'list of Album', func:str)->None:
    list_of_A = AL[:]
    list_of_A.sort(key = func, reverse = True)
    return list_of_A[0]
print("(using favorite_album2)The most popular Album is:\n\n",Album_str(favorite_album2(MUSIC, Album_listening_time)))
print("(using favorite_album2)The most played Album is:\n\n",Album_str(favorite_album2(MUSIC, Album_PT)))
print()
print()
def collection_search(c:'collection',key_word:str)->'list of Songdisplays':
    L = all_Songdisplays(c)
    ret_L = []
    for S in L:
        if key_word in S.s_title or key_word in S.artist or key_word in S.a_title:
            ret_L.append(S)
    return ret_L
print('All Songdisplays containing "Will":')
for i in collection_search(MUSIC,'Will'):
